...
var logger = exports.create_logger('auth0-coreos-tools', ['system']);
...
KafkaStream.prototype._send_pending = function () {
    if (this._ready && this._pending.length > 0) {
        ...
        var messages = [];
        current.forEach(function (entry) {
            messages.push(typeof entry === 'string' ? message : JSON.stringify(entry));
        });
        var payload = [];
        this.options.topics.forEach(function (topic) {
            payload.push({
...
exports.create_logger = function (name, topics) {
    var logger_options = {
        name: name,
        streams: [
            { 
    ...
    if (process.env.KAFKA_ZOOKEEPER) {
        // Enable Kafka stream
        logger_options.streams.push(new KafkaStream({
            zookeeper: process.env.KAFKA_ZOOKEEPER,
            topics: topics
        }));
    }

    return bunyan.createLogger(logger_options);
};

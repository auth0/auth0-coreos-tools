...
// options.host - IP or hostname of the VM the Docker container is running on
...
exports.manage_service = function (options, callback) {
    ...
    ['app_name', 'app_id', 'host', 'image'].forEach(function (i) {
    ...
    if (!options.server) 
        throw new Error('The options.server property must be specified.');
    ...
    async.series([
                    ...
                    container.inspect(function (error, info) {
                        ...
                        if (info.NetworkSettings
                            ...
                            && info.NetworkSettings.Ports[target_port][0].HostPort) 
                                route = options.host + ':' + info.NetworkSettings.Ports[target_port][0].HostPort;
                        if (route)
                            callback();
                        else if (attempts === 0)
        ...
        function (callback) {
            ...
            async.parallel([
                function (callback) {
                    etcd.set(config_path + '/created', Date.now());
                },
                function (callback) {
                    etcd.set(config_path + '/image', options.image);
                },
                function (callback) {
                    etcd.set(config_path + '/host', options.host);
                }
            ], callback);
        },
        function (callback) {
            // Keep container route registration in etcd current with a TTL
            var route_config_path = config_path + '/route';
            async.forever(
                function (next) {

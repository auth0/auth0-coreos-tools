...
exports.webtask = require('./webtask');
        ...
        function (key, callback) {
            etcd.get(path + key, function (error, result) {
                ...
                watch.on('change', function (result) {
                    var modified = watch.current !== result.node.value;
                    if (modified)
                        this.emit('modified', result.node.value);
                    watch.current = result.node.value;
                    if (modified)
                        this.emit('post_modified');
                });
...
exports.etcd_get_consistent = function (key, callback) {
    etcd.get(key, { recursive: true, consistent: true }, function (error, result) {
        error ? callback(error): callback(null, etcd2js(result.node));
    });
};

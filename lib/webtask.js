...
exports.ensure_authenticated = function(ctx) {
    ...
    return function (req, res, next) {
        ...
        function parse_credential(credential) {
            ...
            async.series([
                ...
                function (callback) {
                    ...
                    if(ectx && ectx.JWT_SECRET) {
                        if(!jwt.payload.auth)
                            jwt.payload.auth = {};

                        jwt.payload.auth.secret = ectx.JWT_SECRET;

                        delete ectx.JWT_SECRET;
                    }
                    
                    if(jwt.payload.auth) {
                        if(!req.headers['authorization'] && !req.query.key) {
                            msg = { code: 401, message: 'must provide authorization header or key query', req_id: req.req_id };
                            ctx.logger.error({
                                req_id: req.req_id,
                                path: req.originalUrl,
                                method: req.method,
        ...
        function validate_auth_token(auth) {
            ...
            var userinfo;
            var aud_should_be;
            var secret = auth.secret || ctx.config.default_auth0_client_secret.current;

            if(auth.account)
                aud_should_be = auth.account.client_id;
            else
                aud_should_be = req.headers.host + urlParse(req.url).pathname;

            try {
                secret = new Buffer(secret, 'base64');
            } catch(e) {
                throw { code: 403, message: 'JWT_SECRET must be base64 encoded: ' + e.message, req_id: req.req_id };
            }
            ...
            try {
                ...
                throw { code: 401, message: 'invalid authorization token signature', req_id: req.req_id };
            }

            if(!userinfo.aud)
                throw { code: 403, message: 'no authorization token \'aud\' claim', req_id: req.req_id };

            if(userinfo.aud !== aud_should_be)

...
exports.validate_token_issue_request = function(req, claims) {
    ...
    var fixed_claims = ['url','pctx','ectx','ls','lm','lh','ld','lw','lo'];
...
exports.issue_token = function (options /* req, claims, kid, keyset, store_code_url, store_token */, callback) {
    var new_claims = {
    ...
    async.series([
        ...
        function (cb) {
            if (!options.store_token) return cb();
            var store_path = 'tokens/' + new_claims.jti;
...
exports.ensure_authenticated = function(ctx) {
    ...
    // 3) `jti` request param that maps to a JWT through store_code_url which satisfies #1
    // If JWT is used:
    ...
    return function (req, res, next) {
        var msg;
        if (req.params.jti) {
            var store_path = 'tokens/' + req.params.jti;
            var token = lru.get(store_path);
            if (token) {
                return parse_credential(token);
            }
            ...
            request({
                ...
                if (err) {
                    msg = { code: 502, message: 'unable to resolve jti to webtask token', req_id: req.req_id };
                    ctx.logger.warn({
                        req_id: req.req_id,
                        path: req.originalUrl,
                        method: req.method,
                ...
                if (res1.statusCode !== 200) {
                    msg = { code: res1.statusCode, message: 'unable to resolve jti to webtask token', req_id: req.req_id };
                    ctx.logger.warn({
                        req_id: req.req_id,
                        path: req.originalUrl,
                        method: req.method,
                ...
                if (body && typeof body.token === 'string') {
                    lru.set(store_path, body.token);
                    return parse_credential(body.token);
                }
                msg = { code: 500, message: 'unable to resolve jti to webtask token', req_id: req.req_id };
                ctx.logger.warn({
                    req_id: req.req_id,
                    path: req.originalUrl,
                    method: req.method,

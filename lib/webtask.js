...
var DEFAULT_AUTH0_CLIENT_SECRET = 'SQtMSjGs18nWusU4R0yS3Jdh1Dfk7jdKN5G2t_ItHW8ZrsPu8kp72XIKFtcVzL4r';
var DEFAULT_AUTH0_CLIENT_ID     = 'R5medkAvXYx7CC3DqUVIkWkmbs5NYv7z';
...
exports.ensure_authenticated = function(ctx) {
    ...
    return function (req, res, next) {
                    ...
                    if(jwt.payload.auth) {
                        if(!req.headers['authorization'] && !req.query.key) {
                            ...
                            try {
                                ...
                                msg = { code: 500, message: 'error validating authorization token: ' + e.message, req_id: req.req_id };
                                ctx.logger.error({
                                    req_id: req.req_id,
                                    path: req.originalUrl,
                                    method: req.method,
        ...
        function validate_auth_token(auth) {
            ...
            var userinfo;
            var secret = new Buffer((auth.secret || DEFAULT_AUTH0_CLIENT_SECRET), 'base64');
            ...
            try {
                ...
                throw new Error('Could not parse authorization header.');
            }


            userinfo = jsonwebtoken.verify(auth_token, secret);

            if(userinfo.aud !== (auth.client_id || DEFAULT_AUTH0_CLIENT_ID)) throw new Error('invalid \'aud\' claim');

            if(auth.email) {
                if(userinfo.email_verified === false) {
                    throw new Error('This webtask can only be accessed if your ' + get_provider(userinfo.sub)  + ' email is verified');
                }
                ...
                throw new Error('The email of the account you have attempted to use does not match this webtask\'s authorization policies.');
            } else {
                return userinfo;
            }
        }

...
exports.ensure_authenticated = function(ctx) {
    ...
    return function (req, res, next) {
        ...
        function validate_auth_token(auth) {
            ...
            try {
                secret = new Buffer((auth.secret || ctx.config.default_auth0_client_secret.current), 'base64');
            } catch(e) {
            ...
            if(!auth.secret)
                if(userinfo.aud !== (auth.account ? auth.account.client_id : ctx.config.default_auth0_client_id.current))

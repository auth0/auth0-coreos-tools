...
var AUTH0_CLIENT_SECRET = 'SQtMSjGs18nWusU4R0yS3Jdh1Dfk7jdKN5G2t_ItHW8ZrsPu8kp72XIKFtcVzL4r';
...
exports.ensure_authenticated = function(ctx) {
    ...
    return function (req, res, next) {
        ...
        function parse_credential(credential) {
            ...
            async.series([
                function (callback) {
                    ...
                    if(jwt.payload.auth) {
                        if(!req.headers['authorization'] && !req.query.key) {
                            if(is_browser(req.headers['user-agent']) || req.query.login)
                                jwt.payload.auth = 'pending';
                            else
                                throw new Error('Must specify either authorization header or key query');
                        } else {
                            try {
                                jwt.payload.auth = validate_auth_token(jwt.payload.auth);
                            } catch(e) {
                ...
                function (callback) {
                    ...
                    var ectx;
                    if (jwt.payload.ectx && jwt.payload.auth !== 'pending') {
                ...
                function (callback) {
                    ...
                    if (jwt || jwt.payload.auth === 'pending') return callback();
        ...
        function validate_auth_token(auth) {
            ...
            if(auth.secret) {
               return jsonwebtoken.verify(auth_token, auth.secret);
            } else {
                var secret = new Buffer(AUTH0_CLIENT_SECRET, 'base64');

                var userinfo = jsonwebtoken.verify(auth_token, secret);

                if(!auth.email) {
                    return userinfo;
                } else {
                    if(userinfo.email_verified === false) {
                        throw new Error('User email inverified');
                    }
                    ...
                    throw new Error('User email unauthorized.');
                }
            }
        }
...
function is_browser(useragent) {
    return uaparser(useragent).browser.name;
}

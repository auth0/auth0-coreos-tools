...
exports.issue_token = function (req, claims, kid, keyset, store_code_url, callback) {
    var new_claims = {
        jti: req.jti || uuid.v4().replace(/-/g,''),
    ...
    ['ls','lm','lh','ld','lw','lo'].forEach(function (limit) {
        if (claims[limit] !== undefined)
            new_claims[limit] = claims[limit];
        else if (req[limit] !== undefined)
            new_claims[limit] = req[limit];
    });
    ...
    async.series([
        function (cb) {
            ...
            if (!req.code) return cb();
            var md5 = crypto.createHash('md5');
            var store_path = (claims.jti || 'root') + '/' + md5.update(req.code).digest('hex');
            ...
            request({
                ...
                if (res.statusCode !== 200)
                    return cb(new Error('Error response from store_code_url. HTTP ' + res.statusCode + ': ' + (body || '<unknown>')));
                if (!res.headers['location'])
                    return cb(new Error('Error response from store_code_url. Missing `Location` header.'));

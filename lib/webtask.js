    ...
    return function (req, res, next) {
        ...
        if (req.headers['authorization']) {
            ...
            if (ts.length === 2 && ts[0] === 'Bearer') {
                ...
                // We don't yet know if we are dealing with a named or unnamed webtask so we 
        ...
        //      2) An unnamed webtask with a custom path
        //
        // Since we know that an unnamed webtask request MUST pass in a valid token, we can
        ...
        if (req.params.tenant && req.params.jtn) {
            ...
            // to an unnamed webtask. Note: the token's signature is verified later by
            // `.ensure_authorized`.
            ...
            // for an unnamed webtask. Check to see if the header was set to something that is
            // not a valid bearer-format string. We checked this earlier and put it in `msg`
            // before we were sure if this was unnamed or named.
            if (msg) {
                ctx.logger.warn({
                    req_id: req.req_id,
                    path: req.originalUrl,
            ...
            // This is a legit unnamed webtask. Ready, set, takeoff!
            return parse_credential(jwt);
        }
        else {

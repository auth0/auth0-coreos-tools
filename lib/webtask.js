...
exports.ensure_authorized = function (admin, ctx) {
    return function (req, res, next) {
        ...
        if (!req.claims) {
            ...
            msg = { code: 403, message: 'unauthenticated user is not authorized' };
            ctx.logger.warn({
            ...
            if (req.claims.ten) {
                msg = { code: 403, message: 'insufficient permissions to call administrative API' };
                ctx.logger.warn({
                    path: req.originalUrl,
                    method: req.method,
                    jwt: req._jwt
            ...
            msg = { code: 403, message: 'not authorized to perform operation on tenant `' + req.params.tenant + '`' };
            ctx.logger.warn({
                path: req.originalUrl,
                method: req.method,
                jwt: req._jwt
            ...
            if (jwt) {
                ...
                if (typeof (jwt.header.kid) === 'string' && ctx.config.keysets.set[jwt.header.kid]) {
                    if (jws.verify(credential, ctx.config.keysets.set[jwt.header.kid].signature_key)) {
                        ...
                        if (!isNaN(jwt.claims.nbf) && jwt.claims.nbf > now) {
                            msg = { code: 403, message: 'not yet valid JWT token' };
                            ctx.logger.warn({
                                path: req.originalUrl,
                                method: req.method,
                                credential: credential
                        ...
                        else if (!isNaN(jwt.claims.exp) && jwt.claims.exp < now) {
                            msg = { code: 403, message: 'expired JWT token' };
                            ctx.logger.warn({
                                path: req.originalUrl,
                                method: req.method,
                                credential: credential

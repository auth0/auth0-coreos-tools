...
exports.issue_token = function (options /* req, claims, kid, keyset, store_code_url, store_token, host */, callback) {
    ...
    async.series([
        ...
        function (cb) {
            if(new_claims.auth && !new_claims.jtn)
                return cb(new Error('Can\'t set user authorization for unnamed webtasks.'));
    ...
    return function (req, res, next) {
        ...
        function parse_credential(credential) {
            ...
            async.series([
                ...
                function (callback) {
                    if(jwt.payload.auth) {
                        check_authorization(jwt, function (error) {
                            if(error) {
                                msg = { code: 403, message: error.message, req_id: req.req_id };
                                ctx.logger.warn({
                                    req_id: req.req_id,
                                    path: req.originalUrl,
                                    method: req.method,
        ...
        function check_authorization(jwt, callback) {
            var auth_token;
            var decoded_auth_token;

            if(!req.headers['authorization'] && !req.query.key) {
                msg = { code: 401, message: 'Unauthorized to run webtask. Must provide Authorization header or key query.', req_id: req.req_id };
            ...
            if(jwt.payload.auth.secret) {
                try {
                    auth_token = req.query.key || (req.headers['authorization']
                        .match(/\s.+$/)[0]
                        .replace(/\s/, ''));
                } catch(e) {
                    msg = { code: 401, message: 'Unauthorized to run webtask. Could not parse authorization header.', req_id: req.req_id };
                    return callback(msg);
                }

                try {
                    decoded_auth_token = jsonwebtoken.verify(auth_token, jwt.payload.auth.secret);
                    jwt.payload.auth = decoded_auth_token;
                    return callback();
                } catch(e) {
                    msg = { code: 401, message: 'Unauthorized to run webtask.', req_id: req.req_id };
                    return callback(msg);
                }
            } else {
                // Check against auth0 account
                request({
                    method: 'GET',
                    url: 'https://milomord.eu.auth0.com/userinfo',
                    headers: {
                        Authorization: req.headers['authorization'] || ('Bearer ' + req.query.key)
                    },
                    json: true
                }, function (err, auth0_res) {
                    if(err) {
                        msg = { code: 401, message: 'Unauthorized to run webtask:' + err.message, req_id: req.req_id };
                        return callback(msg);
                    }


                    var userinfo = auth0_res.body;

                    if(userinfo === 'Unauthorized') {
                        msg = { code: 401, message: 'Unauthorized to run webtask. User not found.', req_id: req.req_id };
                        return callback(msg);
                    }

                    if(jwt.payload.auth.email) {
                        if(userinfo.email_verified === false) {
                            msg = { code: 401, message: 'Unauthorized to run webtask. User email unverified.', req_id: req.req_id };

                            return callback(msg);
                        }

                        if(jwt.payload.auth.email instanceof Array) {
                            for(var i = 0; i < jwt.payload.auth.email.length; i++)
                                if(userinfo.email.match(jwt.payload.auth.email[i])) {
                                    jwt.payload.auth.user = userinfo;
                                    return callback();
                                }
                        }
                        else {
                            if(userinfo.email.match(jwt.payload.auth.email)) {
                                jwt.payload.auth = userinfo.email;
                                return callback();
                            }
                        }
                    }

...
exports.ensure_authenticated = function(ctx) {
    ...
    //    (skip this check if a dereferenced token is passed via query `key` or `Authorization` header).
    // If JWT is used:
    ...
    return function (req, res, next) {
        ...
        if (req.headers['authorization']) {
            ...
            if (ts.length === 2 && ts[0] === 'Bearer') {
                ...
                // It is possible that the user is attempting to make a request to a named webtask
                // with an Authorization header that is meant for that named webtask to handle.
                // We don't yet know if we are dealing with a named or anonymous webtask so we 
                // pre-create the error and wait to see what type of request we are dealing with.
                msg = { code: 403, message: 'rejecting request with unexpected Authorization header', req_id: req.req_id };
            }
        }
        else if (req.query.key) {
            jwt = jws.decode(req.query.key);
        }
        
        // A request is coming in that is either:
        //      1) A named webtask; or
        //      2) An anonymous webtask with a custom path
        //
        // Since we know that an anonymous webtask request MUST pass in a valid token, we can
        // disambiguate by checking if the passed-in token's `ten` claim matches the tenant implied
        // by the path (req.params.tenant).
        if (req.params.tenant && req.params.jtn) {
            // If the request comes in with something that can be decoded as a jwt and has a `ten`
            // claim that is consistent with the request's path, we consider this a request
            // to an anonymous webtask. Note: the token's signature is verified later by
            // `.ensure_authorized`.
            if (jwt && jwt.ten === req.params.tenant) {
                return parse_credential(jwt);
            }
            
            // If there is no `ten` claim or that claim doesn't match `req.params.tenant`, this
            // is considered a request to a named webtask.
            var store_path = 'tokens/' + 
            ...
            request({
                ...
                if (err) {
                    msg = { code: 502, message: 'unable to resolve jtn to webtask token', req_id: req.req_id };
                    ctx.logger.warn({
                        req_id: req.req_id,
                        path: req.originalUrl,
                        method: req.method,
            ...
            // We have rejected the request as a named webtask request so it must be a request
            // for an anonymous webtask. Check to see if the header was set to something that is
            // not a valid bearer-format string. We checked this earlier and put it in `msg`
            // before we were sure if this was anonymous or named.
            if (msg) {
                ctx.logger.warn({
                    req_id: req.req_id,
                    path: req.originalUrl,
            ...
            // This is a legit anonymous webtask. Ready, set, takeoff!
            return parse_credential(jwt);
        }
        else {

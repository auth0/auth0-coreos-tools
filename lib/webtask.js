...
exports.ensure_authenticated = function(ctx) {
    ...
    if (!lru) {
        ...
        ctx.config.max_revocation_cache_size.on('modified', function (new_max) {
            lru.max = +new_max;
            ctx.logger.info({ max: +new_max }, 'setting new maximum size of the webtask token LRU cache');
        });
        ctx.config.max_revocation_cache_age.on('modified', function (new_age) {
            lru._maxAge = 1000 * +new_age;
            ctx.logger.info({ max_age: +new_age }, 'setting new maximum age in seconds of the webtask token LRU cache');
        });
    }
        ...
        function parse_credential(credential) {
            ...
            async.series([
                function (callback) {
                    ...
                    if (!revocation_apis) {
                        revocation_apis = revocation.init(ctx);
                    }

                    revocation_apis.check_revocation(req.run_id, jwt, function (error, valid) {
                        if (error) {

...
var DEFAULT_AUTH0_CLIENT_SECRET = 'ywrG3adocXcQIckg0ofmmciLhC6PxJsx9d1UcGjfPtM9Gli2VGhkMs-LB33hEpe5';
var DEFAULT_AUTH0_CLIENT_ID     = 'ODGT86Z8Sx0e92shNVn9N5H8JNGAh8R9';
...
exports.ensure_authenticated = function(ctx) {
    ...
    return function (req, res, next) {
                ...
                function (callback) {
                    ...
                    if(jwt.payload.auth) {
                        if(!req.headers['authorization'] && !req.query.key) {
                            if(wants_html(req))
                                jwt.payload.auth.user = false;
                            else
                                throw new Error('Must specify either authorization header or key query');
                        } else {
                            try {
                                jwt.payload.auth.user = validate_auth_token(jwt.payload.auth);
                            } catch(msg) {
                                ctx.logger.error({
                                    req_id: req.req_id,
                                    path: req.originalUrl,
        ...
        function validate_auth_token(auth) {
            var auth_token;
            ...
            try {
                secret = new Buffer((auth.secret || DEFAULT_AUTH0_CLIENT_SECRET), 'base64');
            } catch(e) {
                throw { code: 400, message: 'error validating authorization token: ' + e.message, req_id: req.req_id };
            }
            ...
            try {
                ...
                throw { code: 400, message: 'could not parse authorization header' + e.message, req_id: req.req_id };
            }
            ...
            try {
                ...
                throw { code: 401, message: 'invalid authorization token signature' + e.message, req_id: req.req_id };
            }

            if(userinfo.aud !== (auth.account ? auth.account.client_id : DEFAULT_AUTH0_CLIENT_ID))
                throw { code: 403, message: 'invalid authorization token \'aud\' claim', req_id: req.req_id };

            if(auth.email) {
                if(userinfo.email_verified === false) {
                    throw { code: 401, message: 'this webtask can only be accessed if your' + get_provider(userinfo.sub) + 'email is verified', req_id: req.req_id };
                }

                if(auth.email instanceof Array) {
                    for(var i = 0; i < auth.email.length; i++)
                        if(userinfo.email.match(auth.email[i])) {
                            return userinfo;
                        }
                }
                else {
                    if(userinfo.email.match(auth.email)) {
                        return userinfo;
                    }
                }

                throw { code: 401, message: 'your email is does not authorize you to view this webtask', req_id: req.req_id };
            } else {
                return userinfo;
            }
        }
...
function wants_html(req) {
    var accept = accepts(req);

    return accept.type(['html']);
}
...
function get_provider(sub) {
    var provider = sub
        .split('|')[0];


    if(provider.match(/\-oauth/)) provider = provider.split('-')[0];

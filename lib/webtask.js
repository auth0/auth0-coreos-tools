    ...
    return function (req, res, next) {
        ...
        if (req.headers['authorization']) {
            ...
            if (ts.length === 2 && ts[0] === 'Bearer') {
                var jwt = jws.decode(ts[1]);
                if (jwt) {
                    // This is a JWT
                    if (typeof (jwt.header.kid) === 'string' && ctx.config.keysets.set[jwt.header.kid]) {
                        if (jws.verify(ts[1], ctx.config.keysets.set[jwt.header.kid].signature_key)) {
                            try {
            ...
            msg = { code: 403, message: 'rejecting request without Authorization header' };
            ctx.logger.warn({
                path: req.originalUrl,
                method: req.method
            }, msg.message);

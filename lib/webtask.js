...
exports.ensure_authenticated = function(ctx) {
    ...
    return function (req, res, next) {
        ...
        function parse_credential(credential) {
            ...
            async.series([
                function (callback) {
                    ...
                    if (!jws.verify(credential, jwt.header.alg, ctx.config.keysets.set[jwt.header.kid].signature_key)) {
        ...
        function validate_authentication_token(secret, expected_aud) {
            ...
            decoded = jws.decode(auth_token);

            if(!decoded)
                throw { code: 400, message: 'malformed authentication token', req_id: req.req_id };

            if(!jws.verify(auth_token, decoded.header.alg, secret))
            ...
            if(decoded.payload.aud !== expected_aud)
                throw { code: 403, message: 'invalid authentication token \'aud\' claim: ' + decoded.aud, req_id: req.req_id };
